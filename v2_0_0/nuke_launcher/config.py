...
class AppConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if not self.base_path.strip():
            errors.append("The base path is missing.")
        elif not Path(self.base_path).is_dir():
            errors.append(f"The base path is not available: {self.base_path}")

        if not self.nuke_executable.strip():
            errors.append("The Nuke executable path is missing.")
        elif not Path(self.nuke_executable).is_file():
            errors.append(f"The Nuke executable was not found: {self.nuke_executable}")
...
class ConfigStore:
    ...
    def load(self) -> AppConfig:
        ...
        try:
            ...
            raise ValueError(f"The configuration could not be read: {exc}") from exc
